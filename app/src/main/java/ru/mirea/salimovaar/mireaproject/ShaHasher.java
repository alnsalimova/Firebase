...
public class ShaHasher {
    private static final String TAG = ShaHasher.class.getSimpleName();
    ...
    public static String hash(String input) {
        byte[] message = input.getBytes(StandardCharsets.UTF_8); //преобразуем входную строку в байты
        final int[] W = new int[64]; //очередь сообщений
        final int[] H = new int[8]; //хеш-значения
        final int[] TEMP = new int[8]; //временные значения

        System.arraycopy(H0, 0, H, 0, H0.length);

        int[] words = pad(message);

        for (int i = 0, n = words.length / 16; i < n; ++i) { //выполняем итерациию сообщения по блокам
        ...
        //byte[] hashBytes = toByteArray(H);

        //String result = new String(hashBytes, StandardCharsets.UTF_8);
        //return result;
    }

    public static int[] pad(byte[] message) { //массив байтов
        int finalBlockLength = message.length % BLOCK_BYTES; //определяем длину конечного блока
        int blockCount = message.length / BLOCK_BYTES + (finalBlockLength + 1 + 8 > BLOCK_BYTES ? 2 : 1); //вычисляем необходимое количество блоков
        ...
        result.position(result.capacity() - 2); //Добавляем к сообщению 1 и заполняем 0, указывая конец сообщения и начало заполнения
